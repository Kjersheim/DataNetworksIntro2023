...
Which, from what I understand, tells the Router B if it wants to send something to that subnet it needs to go by its interface eth1.
...
Setting up a route from Vyos Router A to the subnet 192.168.39.128/26, the new subnet where Lubuntu3 is located., telling it to send traffic in that direction through its interface eth1.
