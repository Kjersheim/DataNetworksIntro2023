...
From the information gained using wireshark, I see new lines within the OSPF fields describing authentication. These update on each hello-packet, and I assume they have the MD5 encryption somehow, 
so I dont immidiately see a way to revert them to ascii. I start an attempt to test them with a program called john, vs an example password file i wrote quickly, but initially it fails. I find some information that using the cain distro could be an alternative to try to see in
 cleartext if I can see the passwords, but at this point I feel the experiment is eating time from the actual exercise so I need to come back to this later. 
