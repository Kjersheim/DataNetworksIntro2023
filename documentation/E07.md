...
Starting up the third Switch, completing the connection of the RING. 
Immidiately I see that the processor is hitting the roof. Lubuntu2 Also suddenly get response from Lubuntu3, but it is very to update on the screen. 
In addition Lubuntu3 sees a hop in response "latency". 

![](/documentation/E07/StartingSwitchC.gif)

### Shutting down Network_Switch_C
And finally when shutting down Switch C, the processes drop immidiately. 
