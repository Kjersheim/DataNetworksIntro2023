...
On the lefthand side of the list we see a list more easily showing just the packets we have filtered and we can use this to go down chronologically. 
Within the info column we see the Transaction ID for the first query and query response. This is also marked within the first query, as well as the actual query to student.labranet.jamk.fi.