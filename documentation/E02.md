...
## Capturing network data on Network_Switch_A
...
It is though not clear to me which devices are mentioned in the final part of the questioning ("from both devices"). 
...
SwitchB.3 # #
SwitchB.3 # configure vlan default delete ports all
* SwitchB.4 # configure vr VR-Default delete ports 1-2
Error: Port 1 belongs to 1 VLAN(s). Delete the port from the VLAN(s)
* SwitchB.5 # configure vr VR-Default add ports 1-2
* SwitchB.13 # show configuration 
#
# Module devmgr configuration.
#
configure snmp sysName "SwitchB"
